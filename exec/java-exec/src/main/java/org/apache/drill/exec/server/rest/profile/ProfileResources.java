  ...
  private QueryProfile getQueryProfile(String queryId) {
    ...
    try {
      ...
      if (profiles instanceof LocalPersistentStore<?>) {
        ...
        final QueryProfile archivedQueryProfile = localProfiles.getArchived(queryId);
        long fetchTime = System.currentTimeMillis() - startOfFetch;
        logger.info("Took {} ms to fetch {}", fetchTime, queryId);
